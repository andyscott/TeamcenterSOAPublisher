...
import com.googlecode.bushel.ivy.internal.FilePackageScanner;
import com.googlecode.bushel.ivy.internal.JarEntryResource;
import com.googlecode.bushel.ivy.internal.JarFileRepository;
import com.googlecode.bushel.util.ZipUtil;
 ...
 */
public class BushelFileResolver extends FileSystemResolver {
    
    private final FilePackageScanner packageScanner = new FilePackageScanner();

    public BushelFileResolver() {
    ...
    protected ResolvedResource findArtifactRef(Artifact artifact, Date date) {
        ...
        if ((resource instanceof FileResource) && ((FileResource) resource).getFile().isDirectory()) {
            ...
            try {
                final File bundleZipFile = File.createTempFile("bushel-" + newMrid, ".zip");
